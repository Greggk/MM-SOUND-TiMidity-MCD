...
SUBDIRS =  	utils 	libarc 	interface 	libunimod 	timidity 	doc


EXTRA_DIST =  	AUTHORS 	COPYING 	ChangeLog 	ChangeLog.1 	INSTALL 	INSTALL.jp 	NEWS 	README 	README.jp 	timidity.ide 	timidity.mak 	TiMidity-uj.ad 	TiMidity.ad
...
common.makefile.in config.guess config.h.in config.sub configure \
configure.in install-sh interface.h.in missing mkinstalldirs
...
stamp-h2: $(srcdir)/interface.h.in $(top_builddir)/config.status
	cd $(top_builddir) \
	  && CONFIG_FILES= CONFIG_HEADERS=interface.h \
	     $(SHELL) ./config.status
	@echo timestamp > stamp-h2 2> /dev/null
$(srcdir)/interface.h.in: $(srcdir)/stamp-h2.in
	@if test ! -f $@; then \
...
TAGS: tags-recursive $(HEADERS) $(SOURCES) config.h.in interface.h.in $(TAGS_DEPENDENCIES) $(LISP)
	tags=; \
	here=`pwd`; \
	list='$(SUBDIRS)'; for subdir in $$list; do \
   if test "$$subdir" = .; then :; else \
	...
	unique=`for i in $$list; do echo $$i; done | \
	  awk '    { files[$$0] = 1; } \
	       END { for (i in files) print i; }'`; \
	test -z "$(ETAGS_ARGS)config.h.in interface.h.in$$unique$(LISP)$$tags" \
	  || (cd $(srcdir) && etags $(ETAGS_ARGS) $$tags config.h.in interface.h.in $$unique $(LISP) -o $$here/TAGS)
...
all-recursive-am: config.h interface.h
